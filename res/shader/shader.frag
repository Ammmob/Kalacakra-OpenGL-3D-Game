...
void main() {
	...
	vec4 specular_color = texture(texture_specular0, v_texCoord);
	vec4 height_color = texture(texture_height0, v_texCoord);
	vec4 normal_color = texture(texture_normal0, v_texCoord);
	// vec3 view_direction = normalize(u_view_position - v_fragPos);

	fragColor = GetDirLight(u_dirLight, object_color, specular_color);

	// fragColor += GetPointLight(u_pointLights, object_color, specular_color);
	if(isSpotLight){
